...
constraints_str = [
    st.text_input(
        f"Constraint {i+1} (e.g. x + y - 1 >= 0, or x + y <= 5, or x - y = 2)",
...
if st.button("Solve"):
    ...
    for i in range(n_vars):
        if lb_inputs[i].strip() == "" and ub_inputs[i].strip() == "":
            if lbs[i] is None:
                lbs[i] = 0.0  # default to nonnegativity
    ...
    # FIX: also include equality constraints in SLSQP
    for (val_fun, jac_fun, _) in eq_trips:
        slsqp_cons.append({
            "type": "eq",
            "fun":  lambda x, f=val_fun:  float(np.asarray(f(x), float).reshape(())),
            "jac":  lambda x, j=jac_fun:  np.asarray(j(x), float).ravel()
    ...
    # Optionally skip SLSQP if there are only equalities (trust-constr handles them well).
    skip_slsqp = (len(ineq_trips) == 0 and len(eq_trips) > 0)
    ...
    if not skip_slsqp:
        try:
            res1 = minimize(
                ...
                options=dict(ftol=1e-9, maxiter=5000, disp=False)  # FIX: slightly relaxed tol
            )
        except Exception as e:
            st.error(f"SLSQP failed to start: {e}")
            st.stop()
    else:
        # dummy object if we skip SLSQP
        class _R: pass
        res1 = _R()
        res1.success = False
        res1.x = x0
    ...
    # ---------- trust-constr with exact Hessians ----------
    nlcs = []
    ...
    # FIX: choose a sane polishing start; only use SLSQP result if near-feasible
    start_for_tc = x0
    if getattr(res1, "success", False) and np.all(np.isfinite(res1.x)) and max_violation(res1.x) < 1e-3:
        start_for_tc = res1.x
    ...
    try:
        res2 = minimize(
            ...
            method="trust-constr",
            jac=g,
            hess=H,                         # exact objective Hessian
            bounds=bounds,
            constraints=nlcs,               # each with exact jac & hess
            options=dict(xtol=1e-9, gtol=1e-9, barrier_tol=1e-12, maxiter=20000, verbose=0),
        )
        # prefer the better feasible candidate
        cand = res2 if (res2.success and res2.fun <= getattr(res1, "fun", np.inf)) else res1
    except Exception:
        cand = res1
